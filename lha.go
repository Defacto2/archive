...
func (x Extractor) Unar(targets ...string) error {
	...
	out, err := cmd.Output()
	if err != nil {
		const filefailOkay = " file failed."
		if strings.Contains(string(out), filefailOkay) {
			return nil
		}
		if buf.String() != "" {
			return fmt.Errorf("extract unar %w: %s: %s", ErrProg, prog, strings.TrimSpace(buf.String()))
		}
		return fmt.Errorf("extract unar %w: %s", err, prog)
	}
	if len(out) == 0 {
		return ErrRead
	}
